...
## 2. 핵심 개발 전략: FRONTEND FIRST (절대 준수)

### 2.1 현재 개발 단계: UI/UX 고도화 전용 단계
- **실제 백엔드 API 연동 금지**: Supabase 실제 통신이나 Python(FastAPI/Django) 백엔드 연결 코드를 작성하지 않습니다.
- **Mock Data 의무화**: 모든 데이터(근무 스케줄, 사용자 프로필, 운세 결과, 친구 목록, 게시판 글 등)는 `src/mocks/` 폴더의 엄격하게 타이핑된 정적 Mock Data와 `src/store/`의 Zustand 모의 상태로만 관리합니다.
...
- **철저한 관심사 분리 (Separation of Concerns, SoC)**:
  ...
  2. **비즈니스 & 상태 계층 (`src/store/`, Custom Hooks)**: 듀티 계산, 필터링, 정렬, 폼 검증 등 복잡한 로직은 반드시 독립된 커스텀 훅(`useXXX`)이나 Zustand 스토어로 추출하여 분리합니다.
  3. **데이터 계층 (`src/mocks/`)**: 모의 데이터 구조와 정적 리터럴은 절대 컴포넌트 파일 내에 인라인으로 방치하지 않고 `src/mocks/`에 엄격한 타입과 함께 별도 모듈로 분리합니다.
  4. **상수 및 유틸 계층 (`src/constants/`, `src/utils/`)**: 색상, 테마, 근무 코드 메타데이터, 공통 헬퍼 함수는 독립 모듈로 중앙 집중화하여 중복 정의를 방지합니다.
- **선제적 컴포넌트 분할 기준**:
...
- **🚫 NO GRADIENTS**: 그라디언트는 일절 사용하지 않으며, 솔리드 컬러 블록(`Solid Color`)만을 사용합니다.
...
### 3.4 브랜드 컬러 준수
- Primary 액센트 컬러: **비바 코랄 핑크 (`#FF507C`)**
- 배경: **순백의 화이트 (`#FFFFFF`)**
- 모든 색상 하드코딩을 지양하고 `src/constants/theme.ts`의 `COLORS` 상수를 참조합니다.
...
2. **타입 무결성 유지**: `any` 타입 사용을 지양하고, 듀티 코드(`ShiftCode: 'D' | 'E' | 'N' | 'O' | 'V'`) 등 표준 타입을 엄격히 준수합니다.
...
> - "백엔드 API나 Supabase를 지금 연결해야 하는가, 아니면 Mock Data를 써야 하는가?"
